...
export class PRCategoryActionNode extends TreeNode implements vscode.TreeItem {
  ...
  constructor(
    ...
    hasUnloadedNotifications?: boolean,
  ) {
    super(parent);
    this.type = type;
...
export function isLocalQuery(queryInfo: IQueryInfo): boolean {
  return (
    queryInfo.label === DefaultQueries.Queries.LOCAL &&
    queryInfo.query === DefaultQueries.Values.DEFAULT
  );
}

export function isAllQuery(queryInfo: IQueryInfo): boolean {
  return (
    queryInfo.label === DefaultQueries.Queries.ALL &&
    queryInfo.query === DefaultQueries.Values.DEFAULT
  );
}
...
export class CategoryTreeNode extends TreeNode implements vscode.TreeItem {
  ...
  constructor(
    ...
    if (
      this._prsTreeModel.expandedQueries === undefined &&
      this.type === PRType.All
    ) {
      this.collapsibleState = vscode.TreeItemCollapsibleState.Expanded;
    } else {
      this.collapsibleState = this._prsTreeModel.expandedQueries?.has(this.id)
    ...
    if (this.isCopilot) {
      this.tooltip = vscode.l10n.t(
        "Pull requests you asked the coding agent to create",
      );
    } else if (this.type === PRType.LocalPullRequest) {
      this.tooltip = vscode.l10n.t(
        "Pull requests for branches you have locally",
      );
    } else if (this.type === PRType.All) {
      this.tooltip = vscode.l10n.t(
        "All open pull requests in the current repository",
      );
    } else if (_categoryQuery) {
      this.tooltip = new vscode.MarkdownString(
        vscode.l10n.t("Pull requests for query: `{0}`", _categoryQuery),
      );
    } else {
  ...
  private _getDescription(): string | undefined {
    ...
    const counts = this._prsTreeModel.getCopilotCounts(
      this._repo.owner,
  ...
  /**
   * Checks if there are notifications for PRs that haven't been loaded yet
   */
  private _hasUnloadedNotifications(): boolean {
    // Get all PR notifications
    const allPrNotifications = this._notificationProvider.prNotifications;

    // Check if any of these notifications are for PRs not in the current loaded set
    for (const notificationPr of allPrNotifications) {
      // Check if this PR is in the currently loaded set
      const isLoaded = Array.from(this.prs.values()).some(
        (loadedPr) =>
          loadedPr.number === notificationPr.number &&
          loadedPr.remote.owner === notificationPr.remote.owner &&
          loadedPr.remote.repositoryName ===
            notificationPr.remote.repositoryName,
      );
  ...
  private async doGetChildren(): Promise<TreeNode[]> {
    ...
    if (this.type === PRType.LocalPullRequest) {
      try {
        this.prs.clear();
        (
          await this._prsTreeModel.getLocalPullRequests(this.folderRepoManager)
        ).items.forEach((item) => this.prs.set(item.id, item));
      } catch (e) {
      ...
      try {
        ...
        if (
          ...
          e.message.includes("the users do not exist")
        ) {
          ...
          vscode.window
            ...
            .then((action) => {
              if (action && action === actions[0]) {
                this.folderRepoManager.credentialStore.recreate(
                  vscode.l10n.t("Your login session is no longer valid."),
                );
              }
            });
          needLogin = e instanceof AuthenticationError;
    ...
    if (this.prs.size > 0) {
      const nodes: (PRNode | PRCategoryActionNode)[] = Array.from(
        this.prs.values(),
      ).map(
        (prItem) =>
          new PRNode(
      ...
      if (hasMorePages) {
        // Check if there are notifications for PRs that haven't been loaded yet
        const hasUnloadedNotifications = this._hasUnloadedNotifications();
        nodes.push(
          new PRCategoryActionNode(
            this,
            PRCategoryActionType.More,
            this,
            hasUnloadedNotifications,
          ),
        );
  ...
  async getTreeItem(): Promise<vscode.TreeItem> {
    ...
    if (!this._repo) {
      this._repo = await extractRepoFromQuery(
        this.folderRepoManager,
        this._categoryQuery,
      );
    }
    this.resourceUri = toQueryUri({
      remote: this._repo,
    ...
    if (this._categoryQuery) {
      const hasNotifications =
        this.isCopilot &&
        this._repo &&
        this._prsTreeModel.getCopilotNotificationsCount(
          this._repo.owner,
          this._repo.repositoryName,
        ) > 0;
      this.contextValue = this.isCopilot
        ? hasNotifications
          ? "copilot-query-with-notifications"
          : "copilot-query"
        : "query";
    }
