	...
	private registerRepositoryChangeEvent() {
		this.gitAPI.repositories.forEach(repository => {
			this.context.subscriptions.push(repository.state.onDidChange(async () => {
				...
				if (state.lastHead && (repository.state.HEAD ? repository.state.HEAD.commit : undefined) !== state.lastHead) {
				...
				if (!state.currentIssue || (newBranch !== state.currentIssue.branchName)) {
					if (newBranch) {
