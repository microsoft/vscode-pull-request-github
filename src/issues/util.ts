		...
		for (const comment of issue.item.comments) {
			if (comment.databaseId === commentNumber) {
				...
				let commentText = marked.parse(
					comment.body.length > ISSUE_BODY_LENGTH ? comment.body.substr(0, ISSUE_BODY_LENGTH) + '...' : comment.body,
					{ renderer: new PlainTextRenderer() },
				);
...
export async function createGithubPermalink(
	...
	return {
		permalink: `https://github.com/${new Protocol(upstream.fetchUrl).nameWithOwner}/blob/${log[0].hash}${pathSegment}#L${
			range.start.line + 1
		}-L${range.end.line + 1}`,
		error: undefined,
	};
