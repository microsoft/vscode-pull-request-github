...
export class IssueFeatureRegistrar extends Disposable {
  ...
  async initialize() {
    ...
    const view = vscode.window.createTreeView("issues:github", {
      ...
      treeDataProvider: new IssuesTreeData(
        this._stateManager,
        this.manager,
        this.context,
      ),
      ...
      vscode.commands.registerCommand(
        ...
        (context: LinkContext, additional: LinkContext[] | undefined) => {
          ...
          return this.copyPermalink(
            this.manager,
            additional && additional.length > 0 ? additional : [context],
          );
        },
        this,
      ),
    ...
    this._register(
      vscode.commands.registerCommand(
        ...
        (fileUri: vscode.Uri, additional: vscode.Uri[] | undefined) => {
          ...
          return this.copyHeadLink(
            additional && additional.length > 0 ? additional : [fileUri],
          );
        },
        this,
      ),
        ...
        (issue: any) => {
          ...
          if (issue instanceof IssueModel) {
            commands.executeCommand(
              chatCommandID,
              vscode.l10n.t(
                "@githubpr Summarize issue {0}/{1}#{2}",
                issue.remote.owner,
                issue.remote.repositoryName,
                issue.number,
              ),
            );
          } else {
    ...
    this._register(
      vscode.commands.registerCommand("issue.chatSuggestFix", (issue: any) => {
        ...
        commands.executeCommand(
          chatCommandID,
          vscode.l10n.t(
            "@githubpr Find a fix for issue {0}/{1}#{2}",
            issue.remote.owner,
            issue.remote.repositoryName,
            issue.number,
          ),
    ...
    this._stateManager.tryInitializeAndWait().then(() => {
      ...
      this._register(
        vscode.languages.registerCodeActionsProvider(
          "*",
          new IssueTodoProvider(this.context, this.copilotRemoteAgentManager),
        ...
        for (const element of providers) {
          if (
            change.affectsConfiguration(
              `${ISSUES_SETTINGS_NAMESPACE}.${element.configuration}`,
            )
          ) {
  ...
  async createIssue() {
    ...
    let folderManager: FolderRepositoryManager | undefined = uri
      ? this.manager.getManagerForFile(uri)
      : undefined;

    const alwaysPrompt = vscode.workspace
      .getConfiguration(ISSUES_SETTINGS_NAMESPACE)
      .get<boolean>(ALWAYS_PROMPT_FOR_NEW_ISSUE_REPO);
    ...
    let remote = remoteName
      ? folderManager.repository.state.remotes.find(
          (r) => r.name === remoteName,
        )
      : undefined;

    if (!remote) {
      const potentialRemotes = folderManager.repository.state.remotes.filter(
        (r) => r.fetchUrl || r.pushUrl,
      );
      ...
      const choice = await vscode.window.showQuickPick(choices, {
        placeHolder: vscode.l10n.t("Select a remote to file this issue to"),
      });
      if (!choice) {
        return;
      }
  ...
  async doStartWorking(
    ...
    if (!repoManager) {
      repoManager = await this.chooseRepo(
        vscode.l10n.t(
          "Choose which repository you want to work on this isssue in.",
        ),
      );
      if (!repoManager) {
        return;
    ...
    const remoteNameResult = await repoManager.findUpstreamForItem({
      githubRepository,
      remote,
    });
  ...
  async startWorking(issue: any) {
    if (issue instanceof IssueModel) {
      return this.doStartWorking(
        this.manager.getManagerForIssueModel(issue),
        issue,
      );
      ...
      const folderManager = this.manager.folderManagers.find((folderManager) =>
        folderManager.gitHubRepositories.find(
          (repo) =>
            repo.remote.owner === parsed?.owner &&
            repo.remote.repositoryName === parsed.name,
        ),
      );
      if (parsed && folderManager) {
        const issueModel = await getIssue(
          this._stateManager,
          folderManager,
          issue.toString(),
          parsed,
  ...
  async stopWorking(issueModel: any) {
    ...
    if (!folderManager) {
      folderManager = await this.chooseRepo(
        vscode.l10n.t(
          "Choose which repository you want to stop working on this issue in.",
        ),
      );
      if (!folderManager) {
        return;
    ...
    if (
      ...
      this._stateManager.currentIssue(folderManager.repository.rootUri)?.issue
  ...
  private async statusBarActions(currentIssue: CurrentIssue) {
    const openIssueText: string = vscode.l10n.t(
      "{0} Open #{1} {2}",
      "$(globe)",
      currentIssue.issue.number,
      currentIssue.issue.title,
    );
    const pullRequestText: string = vscode.l10n.t({
      message: "{0} Create pull request for #{1} (pushes branch)",
      args: ["$(git-pull-request)", currentIssue.issue.number],
      comment: [
        "The first placeholder is an icon and shouldn't be localized",
        "The second placeholder is the ID number of a GitHub Issue.",
      ],
    });
    ...
    const choices =
      currentIssue.branchName && defaults
        ...
        : [openIssueText, pullRequestText, stopWorkingText];
    const response: string | undefined = await vscode.window.showQuickPick(
      choices,
      {
      ...
      const choices: IssueChoice[] = currentIssues.map((currentIssue) => {
        return {
          label: vscode.l10n.t(
            "#{0} from {1}",
            currentIssue.issue.number,
            `${currentIssue.issue.githubRepository.remote.owner}/${currentIssue.issue.githubRepository.remote.repositoryName}`,
          ),
          currentIssue,
        };
      });
      const response: IssueChoice | undefined =
  ...
  async createTodoIssueClipboard() {
    return this.createTodoIssue(
      undefined,
      await vscode.env.clipboard.readText(),
    );
  }

  private async createTodoIssueBody(
    newIssue?: NewIssue,
    issueBody?: string,
    ...
    let contents = "";
    if (newIssue) {
      const folderRepoManager = this.manager.getManagerForFile(
        newIssue.document.uri,
      );
      const changeAffectingFile =
        folderRepoManager?.repository?.state.workingTreeChanges.find(
          (value) => value.uri.toString() === newIssue.document.uri.toString(),
        );
  ...
  async createTodoIssue(newIssue?: NewIssue, issueBody?: string) {
    ...
    if (
      matches &&
      matches.length === 2 &&
      (await this._stateManager.getUserMap(document.uri)).has(matches[1])
    ) {
      assignees = [matches[1]];
    }
  ...
  private async makeNewIssueFile(
    ...
    const repoRef = folderManager.findRepo(
      (githubRepo) => githubRepo.remote.remoteName === options?.remote?.name,
    )?.remote.gitProtocol;
    const repoUrl = repoRef?.url.toString().endsWith(".git")
      ? repoRef?.url.toString().slice(0, -4)
      : repoRef?.url.toString();
    const repoUriParams: RepoUriParams | undefined = repoRef
      ? {
          owner: repoRef?.owner,
          repo: repoRef?.repositoryName,
          repoRootUri: folderManager.repository.rootUri,
        }
      : undefined;
    ...
    if (
      vscode.window.visibleTextEditors.filter(
        (visibleEditor) =>
          visibleEditor.document.uri.scheme === Schemes.NewIssue,
      ).length > 0
    ) {
      return;
    }
    ...
    const assigneeLine = `${ASSIGNEES} ${
      options?.assignees && options.assignees.length > 0
        ? options.assignees.map((value) => "@" + value).join(", ") + " "
        : ""
    }`;
    ...
    const text =
      cached && cached !== ""
        ? cached
        : `${options?.title ?? vscode.l10n.t("Issue Title")}\n
...
<!-- ${vscode.l10n.t('Edit the body of your new issue then click the ✓ \"Create Issue\" button in the top right of the editor. The first line will be the issue title. Assignees and Labels follow after a blank line. Leave an empty line before beginning the body of the issue.')} -->`;
    ...
    const assigneesDecoration = vscode.window.createTextEditorDecorationType({
      after: {
        contentText: vscode.l10n.t(
          " Comma-separated usernames, either @username or just username.",
        ),
        fontStyle: "italic",
        ...
        if (textEditor?.document.uri.scheme === Schemes.NewIssue) {
          ...
          if (labelFullLine.text.startsWith(LABELS)) {
            textEditor.setDecorations(labelsDecoration, [
              new vscode.Range(
                new vscode.Position(labelLineNum, 0),
                new vscode.Position(labelLineNum, labelFullLine.text.length),
              ),
            ]);
          }
          ...
          if (projectsFullLine.text.startsWith(PROJECTS)) {
            textEditor.setDecorations(projectsDecoration, [
              new vscode.Range(
                new vscode.Position(projectsLineNum, 0),
                new vscode.Position(
                  projectsLineNum,
                  projectsFullLine.text.length,
                ),
              ),
            ]);
          }
  ...
  private async verifyLabels(
    ...
    if (newLabels.length > 0) {
      ...
      const promptResult = await vscode.window.showInformationMessage(
        vscode.l10n.t(
          "The following labels don't exist in this repository: {0}. \nDo you want to create these labels?",
          newLabels.join(", "),
        ),
        { modal: true },
        yes,
  ...
  private async chooseRepo(
    prompt: string,
  ): Promise<FolderRepositoryManager | undefined> {
  ...
  private async chooseTemplate(
    ...
    { title: string | undefined; body: string | undefined } | undefined
  > {
    ...
    const choices: IssueChoice[] = templates
  ...
  private async doCreateIssue(
    ...
    if (originUri && originUri.scheme === Schemes.Repo) {
      ...
      if (repoUriParams) {
        origin = {
          owner: repoUriParams.owner,
          repo: repoUriParams.repo,
          base: "",
        };
        folderManager = this.manager.getManagerForFile(
          repoUriParams.repoRootUri,
        );
      }
      if (!folderManager) {
        vscode.window.showErrorMessage(
          vscode.l10n.t(
            `Could not find the correct repository for the issue; see logs for more details.`,
          ),
        );
        Logger.error(
          `Could not find the folder manager for the issue originUri: ${originUri.toString()}`,
          IssueFeatureRegistrar.ID,
        );
        return false;
      }
    }
    ...
    const assigneesWithoutCopilot = assignees?.filter(
      (assignee) => !COPILOT_ACCOUNTS[assignee],
    );
    const copilotAssignee = !!assignees?.find(
      (assignee) => COPILOT_ACCOUNTS[assignee],
    );
    ...
    return vscode.window.withProgress(
      {
        ...
        title: vscode.l10n.t("Creating issue"),
      },
      async (progress) => {
        if (!folderManager) {
          return false;
        ...
        progress.report({
          message: vscode.l10n.t("Verifying that issue data is valid..."),
        });
        try {
          if (!origin) {
          ...
          // There is no remote
          vscode.window.showErrorMessage(
            vscode.l10n.t("There is no remote. Can't create an issue."),
          );
          return false;
        }
        ...
        progress.report({
          message: vscode.l10n.t(
            "Creating issue in {0}...",
            `${createParams.owner}/${createParams.repo}`,
          ),
        });
        const issue = await constFolderManager.createIssue(createParams);
        if (issue) {
          if (copilotAssignee) {
            const copilotUser = (await folderManager.getAssignableUsers())[
              issue.remote.remoteName
            ].find((user) => COPILOT_ACCOUNTS[user.login]);
            if (copilotUser) {
              await issue.replaceAssignees([
                ...(issue.assignees ?? []),
                copilotUser,
              ]);
            }
          ...
          if (
            ...
            const insertText: string =
              vscode.workspace
                .getConfiguration(ISSUES_SETTINGS_NAMESPACE)
                .get(CREATE_INSERT_FORMAT, "number") === "number"
                ? `#${issue.number}`
                : issue.html_url;
            edit.insert(
              document.uri,
            ...
            const openIssue = vscode.l10n.t({
              message: "Open Issue",
              comment:
                "Open the issue description in the editor to see it's full contents.",
            });
            vscode.window
  ...
  private async getPermalinkWithError(
    repositoriesManager: RepositoriesManager,
    includeRange: boolean,
    includeFile: boolean,
    context?: LinkContext[],
  ): Promise<PermalinkInfo[]> {
    const links = await createGithubPermalink(
      repositoriesManager,
      this.gitAPI,
      includeRange,
      includeFile,
    ...
    if (firstError) {
      vscode.window.showWarningMessage(
        vscode.l10n.t(
          "Unable to create a GitHub permalink for the selection. {0}",
          firstError.error!,
        ),
      );
    }
  ...
  private async getHeadLinkWithError(
    context?: vscode.Uri[],
    includeRange?: boolean,
  ): Promise<PermalinkInfo[]> {
    ...
    if (links.length > 0) {
      ...
      if (firstError) {
        vscode.window.showWarningMessage(
          vscode.l10n.t(
            "Unable to create a GitHub link for the selection. {0}",
            firstError.error!,
          ),
        );
      }
  ...
  private async getContextualizedLink(
    file: vscode.Uri,
    link: string,
  ): Promise<string> {
    let uri: vscode.Uri;
  ...
  private async permalinkInfoToClipboardText(
    links: PermalinkInfo[],
    shouldContextualize: boolean = false,
  ): Promise<string | undefined> {
    const withPermalinks: (PermalinkInfo & { permalink: string })[] =
      links.filter(
        (link): link is PermalinkInfo & { permalink: string } =>
          !!link.permalink,
      );
    if (withPermalinks.length !== 0) {
      const contextualizedLinks = await Promise.all(
        withPermalinks.map(async (link) =>
          shouldContextualize && link.originalFile
            ? await this.getContextualizedLink(
                link.originalFile,
                link.permalink,
              )
            : link.permalink,
      ...
      Logger.debug(
        `Will write ${clipboardText} to the clipboard`,
        PERMALINK_COMPONENT,
      );
  ...
  async copyPermalink(
    ...
    contextualizeLink: boolean = false,
  ) {
    const links = await this.getPermalinkWithError(
      repositoriesManager,
      includeRange,
      includeFile,
      context,
    );
    const clipboardText = await this.permalinkInfoToClipboardText(
      links,
      contextualizeLink,
    );
    if (clipboardText) {
    ...
    if (withPermalinks.length === 1) {
      ...
      if (selection) {
        return vscode.env.clipboard.writeText(
          `[${selection.trim()}](${withPermalinks[0].permalink})`,
        );
      }
    }
    const clipboardText = withPermalinks
      .map(
        (link) =>
          `[${basename(link.originalFile?.fsPath ?? "")}](${link.permalink})`,
      )
      .join("\n");
    Logger.debug(
      `writing ${clipboardText} to the clipboard`,
      PERMALINK_COMPONENT,
    );
    return vscode.env.clipboard.writeText(clipboardText);
  }
  ...
  async openPermalink(repositoriesManager: RepositoriesManager) {
    ...
    if (withPermalinks.length > 0) {
      return vscode.env.openExternal(
        vscode.Uri.parse(withPermalinks[0].permalink),
      );
    }
    return undefined;
  }

  async startCodingAgentFromTodo(todoInfo?: {
    document: vscode.TextDocument;
    lineNumber: number;
    line: string;
    insertIndex: number;
    ...
    if (!todoText) {
      vscode.window.showWarningMessage(
        vscode.l10n.t("No task description found in TODO comment"),
      );
      return;
    }
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        vscode.l10n.t(
          "Failed to start coding agent session: {0}",
          error.message,
        ),
      );
    }
