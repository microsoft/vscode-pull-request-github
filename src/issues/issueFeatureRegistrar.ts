...
export class IssueFeatureRegistrar implements vscode.Disposable {
	...
	async startWorking(issue: any) {
		...
		if (folderManager) {
			await this._stateManager.setCurrentIssue(folderManager, new CurrentIssue(issue, folderManager, this._stateManager));
		}
	}
	...
	async startWorkingBranchPrompt(issueModel: any) {
		...
		if (folderManager) {
			await this._stateManager.setCurrentIssue(folderManager, new CurrentIssue(issueModel, folderManager, this._stateManager, true));
		}
	}
