...
export class IssueFeatureRegistrar implements vscode.Disposable {
	...
	constructor(
		...
		public reviewManagers: ReviewManager[],
	...
	private async statusBarActions(currentIssue: CurrentIssue) {
		...
		const choices =
			currentIssue.branchName && defaults
				? [openIssueText, pullRequestText, draftPullRequestText, stopWorkingText]
				: [openIssueText, pullRequestText, draftPullRequestText, stopWorkingText];
	...
	private async makeNewIssueFile(originUri: vscode.Uri, title?: string, body?: string, assignees?: string[] | undefined) {
		...
		if (
			vscode.window.visibleTextEditors.filter(visibleEditor => visibleEditor.document.uri.scheme === NEW_ISSUE_SCHEME)
				.length > 0
		) {
			return;
		}
			...
			if (textEditor?.document.uri.scheme === NEW_ISSUE_SCHEME) {
				...
				if (assigneeFullLine.text.startsWith(ASSIGNEES)) {
					textEditor.setDecorations(assigneesDecoration, [
						new vscode.Range(new vscode.Position(2, 0), new vscode.Position(2, assigneeFullLine.text.length)),
					]);
				}
		...
		if (issue) {
			if (document !== undefined && insertIndex !== undefined && lineNumber !== undefined) {
				...
				const insertText: string =
					vscode.workspace.getConfiguration(ISSUES_CONFIGURATION).get('createInsertFormat', 'number') === 'number'
						? `#${issue.number}`
						: issue.html_url;
