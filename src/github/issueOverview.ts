...
export class IssueOverviewPanel {
	...
	protected _pullRequestManager: FolderPullRequestManager;
	...
	public static async createOrShow(extensionPath: string, pullRequestManager: FolderPullRequestManager, issue: IssueModel, descriptionNode: DescriptionNode, toTheSide: Boolean = false) {
	...
	protected constructor(extensionPath: string, column: vscode.ViewColumn, title: string, pullRequestManager: FolderPullRequestManager, descriptionNode: DescriptionNode, type: string = IssueOverviewPanel._viewType) {
	...
	private async addLabels(message: IRequestMessage<void>): Promise<void> {
		try {
			...
			async function getLabelOptions(prManager: FolderPullRequestManager, issue: IssueModel): Promise<vscode.QuickPickItem[]> {
